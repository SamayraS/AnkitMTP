...
from data_indore import build_indore_city_data
...
def main():
    ...
    if config.USE_REAL_DATA:
        print("\n[STEP 1] Loading REAL Indore data (from CSVs & OSM)...")
        try:
            data = build_indore_city_data(config)
        except Exception as e:
            print(f"\n[ERROR] Failed to load real data: {e}")
            print("Please ensure all required CSV files are present in the data_real directory.")
            print("Or set USE_REAL_DATA = False in config.py to use synthetic data.")
            return
    else:
        print("\n[STEP 1] Loading SYNTHETIC/HYBRID Indore data...")
        data_loader = IndoreDataLoader(
            city_center_lat=config.CITY_CENTER_LAT,
            city_center_lon=config.CITY_CENTER_LON,
            city_radius_km=config.CITY_RADIUS_KM
        )
    ...
    optimizer = HybridOptimizer(
        data=data,
        nsga2_generations=config.NSGA2_GENERATIONS,
        benders_iterations=config.BENDERS_ITERATIONS
    )

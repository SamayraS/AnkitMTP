...
class IndoreDataLoader:
    ...
    def __init__(self, city_center_lat: float = 22.7196, 
        ...
        target_year : int
            Year for demand projection (default: 2021)
        """
        ...
        self.base_year = 2021
        self.cagr = 0.0704  # 7.04% annual growth based on census
    ...
    def apply_cagr(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Apply CAGR to project population and demand to target year.
        
        Formula: P_t = P_base * (1 + r)^(t - base)
        """
        if self.target_year <= self.base_year:
            return df
            
        years_diff = self.target_year - self.base_year
        growth_factor = (1 + self.cagr) ** years_diff
        
        print(f"  - Applying CAGR projection for year {self.target_year} (factor: {growth_factor:.3f})...")
        
        if 'population' in df.columns:
            df['population'] = df['population'] * growth_factor
            
        if 'demand' in df.columns:
            df['demand'] = df['demand'] * growth_factor
    ...
    def load_real_demand_zones(self) -> pd.DataFrame:
        ...
        try:
            ...
            # Apply CAGR
            self.demand_zones = self.apply_cagr(self.demand_zones)
